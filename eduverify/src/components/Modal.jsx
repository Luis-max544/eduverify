  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-md p-4 animate-fade-in" onClick={onClose}>
      <div
        ...
        className={`w-full ${maxWidth} max-h-[85vh] overflow-y-auto p-6 rounded-[2rem] border shadow-2xl ${darkMode ? 'bg-gray-900 border-white/10 text-white' : 'bg-white border-[var(--clr-border-subtle)] text-gray-900'}`}
      >
        {(title || Icon) && (
          <div className="flex justify-between items-center pb-4 border-b border-gray-100 dark:border-white/5 mb-5">
            <div className="flex items-center gap-2.5 min-w-0">
              {Icon && <Icon size={20} className="text-cyan-500 shrink-0" />}
              {title && <h3 className="text-sm font-black uppercase tracking-wider text-cyan-600 dark:text-cyan-400 truncate">{title}</h3>}
            </div>
