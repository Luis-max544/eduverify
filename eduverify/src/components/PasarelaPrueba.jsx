    ...
    <div className="max-w-md mx-auto animate-fade-in pb-20 select-none">
      ...
      {esPremium ? (
        <div className={darkMode ? "p-6 md:p-8 rounded-3xl border space-y-6 bg-gray-900 border-white/5 text-white" : "p-6 md:p-8 rounded-3xl border space-y-6 bg-white border-gray-200 shadow-xl"}>
          ...
          <button type="button" onClick={() => alert("Generando recibo oficial PDF...")} className={darkMode ? "w-full py-2.5 rounded-xl text-xs font-bold bg-gray-950 border border-white/10 text-white inline-flex items-center justify-center gap-1.5" : "w-full py-2.5 rounded-xl text-xs font-bold bg-white border border-gray-300 text-gray-700 inline-flex items-center justify-center gap-1.5"}><FileText size={14} /> Recibo PDF</button>
        </div>
      ) : (
        <div className={darkMode ? "p-6 md:p-8 rounded-3xl border text-center space-y-5 bg-gray-900 border-white/5 text-white" : "p-6 md:p-8 rounded-3xl border text-center space-y-5 bg-white border-gray-200 shadow-xl"}>
          <Gem size={36} className="mx-auto mb-2 text-blue-500" />
          ...
          <button onClick={activarMembresia} className="w-full bg-blue-600 text-white font-bold py-3.5 rounded-xl text-xs uppercase tracking-widest shadow-md inline-flex items-center justify-center gap-1.5"><CreditCard size={14} /> Activar Membresía de Prueba</button>
        </div>
      )}
    </div>
  );
