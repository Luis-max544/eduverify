      ...
      {loading ? (
        ...
        <div className="space-y-4 text-center">
          ...
          <div className="flex gap-2.5 pt-2">
            ...
            {!resultado.passed && (
              <button onClick={() => { setResultado(null); setRespuestas(Object.fromEntries(quiz.preguntas.map(p => [p.id, null]))); }} className="flex-1 px-4 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest bg-blue-600 text-white shadow-md hover:bg-blue-500 transition-colors">Reintentar</button>
            )}
          </div>
        </div>
      ) : quiz ? (
        <div className="space-y-5">
          ...
          <div className="space-y-4">
            {quiz.preguntas.map((p, pi) => (
              <div key={p.id} className={`p-4 rounded-2xl border ${respuestas[p.id] !== null ? darkMode ? 'border-blue-500/30 bg-blue-500/5' : 'border-blue-300 bg-blue-50' : 'border-gray-100 dark:border-white/5'}`}>
                  ...
                  {p.opciones.map((opt, oi) => (
                    <label key={oi} className={`flex items-center gap-2.5 p-2.5 rounded-xl cursor-pointer transition-colors text-xs font-bold ${
                      respuestas[p.id] === oi
                        ...
                        : darkMode ? 'hover:bg-white/5 text-gray-400' : 'hover:bg-gray-50 text-gray-600'
                    }`}>
                      <input type="radio" name={`q_${p.id}`} checked={respuestas[p.id] === oi} onChange={() => selectAnswer(p.id, oi)} className="w-4 h-4 accent-blue-600 cursor-pointer" />
                      {opt}
                    </label>
                  ))}
                </div>
          ...
          <button
            ...
            className="w-full font-bold py-3.5 rounded-xl text-xs uppercase tracking-widest bg-blue-600 hover:bg-blue-500 disabled:opacity-40 text-white shadow-md transition-colors"
          >
