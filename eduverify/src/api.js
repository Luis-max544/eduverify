...
export const videos = {
  ...
  createWithUpload: (meta) => request('/videos', { method: 'POST', body: { ...meta, upload: true } }),
  uploadUrl: (id) => `${BASE}/api/videos/${id}/upload`,
  streamUrl: (id) => `${BASE}/api/videos/${id}/stream`,
  subtitlesUrl: (id) => `${BASE}/api/videos/${id}/subtitles`,
  uploadThumbnail: (id, file) => {
    const f = new FormData(); f.append('thumbnail', file);
    return request(`/videos/${id}/thumbnail`, { method: 'PUT', body: f, isForm: true });
  },
  uploadSubtitles: (id, file) => {
    const f = new FormData(); f.append('subtitles', file);
    return request(`/videos/${id}/subtitles`, { method: 'PUT', body: f, isForm: true });
  },
...
export function uploadVideoToMinio(uploadEndpointUrl, file, onProgress) {
  return new Promise((resolve, reject) => {
    ...
    xhr.open('POST', uploadEndpointUrl);
